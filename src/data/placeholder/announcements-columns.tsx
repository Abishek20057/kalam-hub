...
import { Announcement, ContentStatus } from "@/lib/types/cms";
...
function ActionsCell({ item }: { item: Announcement }) {
  "use no memo";
  ...
  return (
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem asChild>
            <Link href={`/admin/content/announcements/${item.id}`}>
      ...
      <DeleteDialog isOpen={isDeleteDialogOpen} onClose={() => setIsDeleteDialogOpen(false)} onConfirm={() => { console.log("Deleting..."); setIsDeleteDialogOpen(false); }} title={`Delete "${item.title}"?`} />
    </>
  );
}
...
export const columns: ColumnDef<Announcement>[] = [
  ...
  },
  {
    accessorKey: "type",
    header: "Type",
    cell: ({ row }) => <Badge variant="category" className="capitalize">{row.getValue("type")}</Badge>,
  },
  {
    accessorKey: "status",
    header: "Status",
