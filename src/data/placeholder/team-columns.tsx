...
function ActionsCell({ item }: { item: TeamMember }) {
  "use no memo";
  ...
  return (
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem asChild>
            <Link href={`/admin/content/team/${item.id}`}>
      ...
      <DeleteDialog isOpen={isDeleteDialogOpen} onClose={() => setIsDeleteDialogOpen(false)} onConfirm={() => { console.log("Deleting..."); setIsDeleteDialogOpen(false); }} title={`Delete "${item.name}"?`} />
    </>
  );
}
